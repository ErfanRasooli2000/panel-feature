<template>
  <b-card>
    <b-card-header header-bg-variant="info">
      <b-card-title>ویرایش آلبوم</b-card-title>
    </b-card-header>
    <b-card-body>
        ...
        <b-row>
          <b-col sm="12">
            <b-button
                ...
                @click="updateAlbum"
                :disabled="onProgress"
            >{{!onProgress ? 'ویرایش آلبوم' : 'در حال ارسال اطلاعات'}}</b-button>
          </b-col>
        </b-row>
      </b-form>
    </b-card-body>
...
export default {
  ...
  methods: {
    async updateAlbum(){
      await this.$http.put(`albums/update/${this.$route.params.id}` , {
        name_fa : this.nameFa,
        name_en : this.nameEn,
      })
          .then(({data}) => {
            ...
            else
            {
              this.$bvToast.toast(data.message, {
                title: 'در ویرایش آلبوم مشکلی به وجود آمده.',
    ...
    async getAlbum(){
      await this.$http.get(`albums/show/${this.$route.params.id}`)
          .then(response => {
