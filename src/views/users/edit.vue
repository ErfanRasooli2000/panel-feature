    ...
    <b-card-body>
      ...
      <b-col sm="12">
        <b-button
            ...
            @click="updateUser"
            :disabled="onProgress"
        >{{!onProgress ? 'ویرایش کاربر' : 'در حال ارسال اطلاعات'}}</b-button>
      </b-col>
    </b-card-body>
  </b-card>
</template>
...
export default {
  ...
  methods: {
    async getUser()
    {
      await this.$http.get(`user/show/${this.$route.params.id}`)
          .then(({data}) => {
            if (data.status)
            {
    ...
    async updateUser()
    {
      ...
      await this.$http.post(`user/update/${this.$route.params.id}`, formData , { headers: { 'Content-Type': 'multipart/form-data' }})
          .then(({data}) => {
            if (data.status)
            {
              this.$bvToast.toast('کاربر با موفقیت ذخیره شد.', {
